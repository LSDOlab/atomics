...
residual_form = df.inner(sigm, v_ij) * df.dx + \
    df.dot(C*KAPPA* df.grad(temperature_function),  df.grad(T_hat)) * df.dx
print("get a-------")
...
V = density_function.function_space()
...
von_Mises_form = (1/df.CellVolume(mesh)) * von_Mises * df.TestFunction(density_function.function_space()) * df.dx
...
# der_von = df.derivative(von_Mises_form, von_Mises, df.TrialFunction(mixed_fs))
# der_von_val = df.assemble(der_von).array()
print('the value of the der_val is:', der_val.max())
print('the value of the der_val is:', der_val.min())
print('the norm of the der_val is:', np.linalg.norm(der_val))

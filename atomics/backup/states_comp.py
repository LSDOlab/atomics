...
class StatesComp(om.ImplicitComponent):
    ...
    def setup(self):
        ...
        for argument_name in pde_problem.states_dict[state_name]['arguments']:
            argument_functions_dict[argument_name] = pde_problem.inputs_dict[
        ...
        for argument_name, argument_function in iteritems(
            ...
            self.add_input(argument_name,
                           shape=argument_function.function_space().dim())
        self.add_output(state_name,
                        shape=state_function.function_space().dim())

        dR_dstate = self.compute_derivative(state_name, state_function)
        self.declare_partials(state_name,
                              state_name,
                              rows=dR_dstate.row,
                              cols=dR_dstate.col)
    ...
    def compute_derivative(self, arg_name, arg_function):
        ...
        derivative_petsc_sparse = df.as_backend_type(
            df.assemble(derivative_form)).mat()
    ...
    def solve_nonlinear(self, inputs, outputs):
        ...
        print('prev.size________', prev.size)
        # if (now.size) == 0:
        #     state_all = state_function.vector().get_local()
        #     print(argument_name)
        #     print(inputs[argument_name].mean())
        #     np.savetxt(
        #         '/home/jyan_linux/Downloads/Software/ozone2/ozone2/Examples/steady_state_TO/states.out',
        #         state_all)
        if prev.size != now.size:
            ...
            for argument_name, argument_function in iteritems(
                ...
                print(argument_name)
                print(inputs[argument_name].mean())
                print(inputs[argument_name].shape)

                # save states####################
                states_record = np.loadtxt('states.out')
                state_all = np.vstack(
                    (states_record, state_function.vector().get_local()))
                np.savetxt('states.out', state_all)

                print('state_all:', state_all.shape)
                print('state_name is:', state_name,
                      state_function.vector().get_local().mean())
                print(state_function.vector().get_local().shape)

                # save densities###################
                densities_record = np.loadtxt('densities.out')
                densities_all = np.vstack(
                    (densities_record, inputs[argument_name]))
                np.savetxt('densities.out', densities_all)
        else:

...
from atomics.pdes.thermo_mechanical_convection import get_residual_form
...
class BottomBoundary(df.SubDomain):
    def inside(self, x, on_boundary):
        return (abs(x[1] - 0.) < df.DOLFIN_EPS_LARGE*1e15 and abs(x[0] - LENGTH_X / 2) < 2. * LENGTH_X / NUM_ELEMENTS_X)
...
upper_edge.mark(sub_domains, 6)
dss = df.Measure('ds')(subdomain_data=sub_domains)
f = df.Constant((0, -0/(4. * LENGTH_X / NUM_ELEMENTS_X)))
...
driver.opt_settings['Major optimality tolerance'] =2.e-11

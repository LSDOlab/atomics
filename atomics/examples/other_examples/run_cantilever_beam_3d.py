...
mesh = df.BoxMesh.create(
    [df.Point(0.0, 0.0, 0.0), df.Point(LENGTH_X, LENGTH_Y, LENGTH_Z)],
...
class TractionBoundary(df.SubDomain):
    def inside(self, x, on_boundary):
        return ((abs(x[1] - LENGTH_Y/2) < LENGTH_Y/NUM_ELEMENTS_Y + df.DOLFIN_EPS) 
...
f = df.Constant((0, -1. / 2 / LENGTH_Y , 0.0))
...
pde_problem.add_bc(df.DirichletBC(displacements_function_space, df.Constant((0.0, 0.0, 0.0)), '(abs(x[0]-0.) < DOLFIN_EPS)'))
...
driver.opt_settings['Major optimality tolerance'] =2.e-8

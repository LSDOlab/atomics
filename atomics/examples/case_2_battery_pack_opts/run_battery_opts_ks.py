...
from atomics.ks_stress_comp import KSStressComp
...
with pygmsh.occ.Geometry() as geom:
    geom.characteristic_length_min = 0.006
    geom.characteristic_length_max = 0.006
    disk_dic = {}
    disks = []
...
von_Mises_max = df.project(von_Mises, density_function_space).vector().get_local().max()
von_Mises_form = (1/df.CellVolume(mesh)) * von_Mises * df.TestFunction(density_function_space) * df.dx
# pde_problem.add_field_output('von_Mises', von_Mises_form, 'mixed_states', 'density')
rho_ks = 50.
von_Mises_sum = (
    1/df.CellVolume(mesh) * df.exp(rho_ks * von_Mises)
    ) * df.dx
...
prob.model.add_subsystem('KSStressComp', comp, promotes=['*'])
...
if objective == 'mass':
    ...
    # prob.model.add_constraint('von_Mises_max',upper=1.2e8/scalar_)
    prob.model.add_constraint('von_mises_max',upper=1.2e8/scalar_)
...
df.File('solutions/case_2/battter_packks_{}/displacements.pvd'.format(objective)) << displacements_function_val
df.File('solutions/case_2/battter_packks_{}/temperature.pvd'.format(objective)) << temperature_function_val
df.File('solutions/case_2/battter_packks_{}/density.pvd'.format(objective)) << density_function
stiffness  = df.project(density_function/(1 + 8. * (1. - density_function)), density_function_space) 
df.File('solutions/case_2/battter_pack_ks{}/stiffness.pvd'.format(objective)) << stiffness

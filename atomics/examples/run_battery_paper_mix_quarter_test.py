...
with pygmsh.occ.Geometry() as geom:
    ...
    for i in range(num_cells):
        name = 'disk' + str(i)
        disk_dic[name] = geom.add_disk(
            [xv.flatten()[i], yv.flatten()[i], 0.], 0.01)
    ...
    geom.boolean_difference(
        rectangle, geom.boolean_union([disks, rectangle_1, rectangle_2]))
...
class HeatBoundaryAll(df.SubDomain):
    def inside(self, x, on_boundary):
        cond_list = []
        for i in range(num_cells):
            cond = (abs((x[0] - (xv.flatten()[i]))**2 +
                        (x[1] - (yv.flatten()[i]))**2) <
                    (radius**2) + df.DOLFIN_EPS)
...
bc_temperature = df.DirichletBC(mixed_fs.sub(1), df.Constant(T_0),
                                SurroundingBoundary())
...
for i in [12, 13, 14, 17, 18, 19, 22, 23, 24]:
    idx = tree.query_ball_point(
        list(np.array([xv.flatten()[i], yv.flatten()[i]])), radius + 2e-3)
    idx_list.extend(idx)
...
comp = ExtractComp(
    ...
    in_shape=pde_problem.states_dict['mixed_states']
    ['function'].function_space().dim(),
...
prob.model.add_constraint('density',
                          upper=1.,
                          lower=1.,
                          indices=idx_array,
                          linear=True)
...
df.File('solutions/displacements_function_val_t55_lrf.pvd'
        ) << displacements_function_val
df.File('solutions/temperature_function_val_t55_lrf.pvd'
        ) << temperature_function_val
